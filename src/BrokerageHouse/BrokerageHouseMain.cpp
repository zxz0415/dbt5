...
char szDBPwd[iMaxPwd + 1] = "";
char szDBUser[iMaxUser + 1] = "";
	...
	for (arg = 1; arg < argc; ++arg) {
		...
		*sp = (char) tolower(*sp);
		...
		case 'h': // Database host name.
			...
			break;
		case 'u': // Database host name.
			strncpy(szDBUser, vp, iMaxUser);
			szDBUser[iMaxUser] = '\0';
			break;
		case 'm': // Database host name.
			strncpy(szDBPwd, vp, iMaxPwd);
			szDBPwd[iMaxPwd] = '\0';
			break;
...
int main(int argc, char *argv[])
{
	...
	CBrokerageHouse	BrokerageHouse(szHost, szDBName, szDBPort, szDBPwd, szDBUser, iListenPort,
			outputDirectory);
